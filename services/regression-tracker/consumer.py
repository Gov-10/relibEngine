"""Standalone background consumer for analysis-events (Regression Tracker).

Run:  python consumer.py
Env:  KAFKA_BOOTSTRAP_SERVERS (default localhost:9092)
      ANALYSIS_EVENTS_TOPIC   (default analysis-events)
      KAFKA_CONSUMER_GROUP    (default regression-tracker-service)
      REGRESSION_SERVER_URL / REGRESSION_DB_NAME / REGRESSION_DATABASE_URL
"""
...
logger = logging.getLogger("regression-tracker.consumer")
...
def get_consumer_group() -> str:
    return os.getenv("KAFKA_CONSUMER_GROUP", "regression-tracker-service")
...
def run_consumer() -> None:
    ...
    store.init()
    logger.info("regression store ready")
    ...
    logger.info("regression tracker consumer stopped cleanly")
